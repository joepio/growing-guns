...
| `gamenight_bridge.gd` | Autoload bridging the GameNight daemon to the scene lifecycle (prepare → ready → start → dispose). | Inert unless `GameNight.launched_by_daemon`. Owns who is playing: local seats become splitscreen players bound to real pads at `start` (not at `prepare` — warm runs backgrounded, where pads may not be enumerated), `ai` seats become bots. | 
...
| `splitscreen_manager.gd` | Couch-coop layer: per-gamepad spawn + SubViewports + per-device pause menu. | Child of Game; Game calls `enable_from_meta()` (reads `NetworkManager` meta `splitscreen_on_start`) *after* spawning the host player — doing it from `_ready` is too early, the host doesn't exist yet. Under GameNight the party owns the seats: `bind_seats(devices)` is the only way in, and press-X/click-to-join is off. RPCs (`_do_spawn`, `_despawn`, `_broadcast_scores`) stay on Game; manager calls `_game._do_spawn.rpc(...)`. |
