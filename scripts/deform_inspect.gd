...
# Visual inspector for GPU brick deformation. Renders a row of single bricks, each
# a correct per-body damage setup (its own viewport + material + uniforms), hit at a
# different spot, so the look of a centre/corner/side/heavy hit can be compared at a
# glance. Unlike the render bench, the per-body mapping here is CORRECT.
#
...
func _ready() -> void:
	_build_world()
	# Each case: a label, a hit point in normalized [-1,1] brick coords, a face normal,
	# and a damage multiplier (drives strength + how many accumulating splats).
	var cases := [
		{"pos": Vector3(0.0, 0.0, 1.0), "nrm": Vector3(0, 0, 1), "dmg": 1.0},    # centre, weak
		{"pos": Vector3(0.0, 0.0, 1.0), "nrm": Vector3(0, 0, 1), "dmg": 3.0},    # centre, heavy
		{"pos": Vector3(0.9, 0.9, 1.0), "nrm": Vector3(0, 0, 1), "dmg": 3.0},    # corner, heavy
		{"pos": Vector3(1.0, 0.3, 0.3), "nrm": Vector3(1, 0, 0), "dmg": 1.0},    # side (right face)
	]
	var x0 := -float(cases.size() - 1) * 0.5 * SPACING
	for i in cases.size():
		var c: Dictionary = cases[i]
		_make_brick(Vector3(x0 + float(i) * SPACING, 0.0, 0.0),
			(c["pos"] as Vector3), (c["nrm"] as Vector3), float(c["dmg"]))
	print("[deforminspect] order L→R: centre | corner | side | heavy-corner")
	_run_capture()
...
func _make_brick(world_pos: Vector3, hit_norm_pos: Vector3, hit_normal: Vector3, dmg: float) -> void:
	...
	mat.set_shader_parameter("dmg_crater_depth", 0.85)
	...
	mm.mesh = DestructibleSolid._build_hand_hewn_stone_mesh(SUBDIV)
	mm.instance_count = 1
	...
	# Local hit point in metres (normalized coord * half-extent).
	var local := hit_norm_pos * (BRICK * 0.5)
	var shots := 1 if dmg < 2.0 else 5
	for _k in shots:
		_splat(sp, local, hit_normal, dmg)


func _splat(sp: Splat2D, local_pos: Vector3, n: Vector3, dmg: float) -> void:
	...
	if region < 2:
		ruv = Vector2(t.z, t.y)
		plane = Vector2(BRICK, BRICK)
	elif region < 4:
		ruv = Vector2(t.x, t.z)
	var col := region % 3
	var row := region / 3
	var world_radius := 0.7 * pow(dmg, 0.5)
	sp.pending.append({
		"uv": Vector2((float(col) + ruv.x) / 3.0, (float(row) + ruv.y) / 2.0),
		"ru": (world_radius / plane.x) / 3.0,
		"rv": (world_radius / plane.y) / 2.0,
		"strength": clampf(0.55 + dmg * 0.18, 0.5, 1.5),
	})
	sp.queue_redraw()
...
func _brush() -> Texture2D:
	...
	for y in n:
		for x in n:
			...
			var d := p.length() * 2.0
			var core := clampf(1.0 - d * 1.05, 0.0, 1.0)
			core = core * core
...
func _run_capture() -> void:
	var size := Vector2i(1500, 560)
	DisplayServer.window_set_size(size)
	get_viewport().size = size
	var width := 4.0 * SPACING
